...
/* } */
/*  */
/* __kernel void _kernel_1(__global int * _local_buffer_A,__global const int * _local_buffer_B,int _local_i_start,int _local_i_end,int _loop_step) { */
...
/* } */
/* } */
const char *_kernel_1_src = "void _GfnBarrier() {""    barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);""}""""int _GfnAtomicAddInt(__global int* const address, const int value) {""    return atomic_add(address, value);""}""""float _GfnAtomicAddFloat(__global float* const address, const float value) {""    uint oldval, newval, readback;""    *(float*)&oldval = *address;""    *(float*)&newval = (*(float*)&oldval + value);""    while ((readback = atom_cmpxchg((__global uint*)address, oldval, newval)) != oldval) {""        oldval = readback;""        *(float*)&newval = (*(float*)&oldval + value);""    }""    return *(float*)&oldval;""}""""__kernel void _kernel_1(__global int * _local_buffer_A,__global const int * _local_buffer_B,int _local_i_start,int _local_i_end,int _loop_step) {""int * A = ((void *)_local_buffer_A) - (_local_i_start * sizeof(int));""int * B = ((void *)_local_buffer_B) - (_local_i_start * sizeof(int));""int _loop_size = (_local_i_end - _local_i_start) / _loop_step;""int i = (get_global_id(0) * _loop_step) + _local_i_start;""if (i <= (300) - 1) {""{""    A[(i)] = B[(i)];""    _GfnBarrier();""    A[(i)] *= - 1;""}""}""}";
void _Function_1()
{
    int i;
    int *A;
    ...
    B = ((void *) _local_buffer_B) - ((_local_i_start * ((1) * (1))) * sizeof(int));
