...
/* } */
/*  */
/* __kernel void _kernel_1(int n,double h,__global double * sum,__local double * _cl_local_mem_sum,int _local_i_start,int _local_i_end,int _loop_step) { */
/* int n; */
/* double x; */
/* double h; */
/* double sum = 0; */
...
/* if (get_local_id(0) == 0) { */
/* _GfnAtomicAddDouble(_local_buffer_sum,_cl_local_mem_sum[0]); */
/* } */
/* } */
const char *_kernel_1_src = "void _GfnBarrier() {""    barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);""}""""int _GfnAtomicAddInt(__global int* const address, const int value) {""    return atomic_add(address, value);""}""""float _GfnAtomicAddFloat(__global float* const address, const float value) {""    uint oldval, newval, readback;""    *(float*)&oldval = *address;""    *(float*)&newval = (*(float*)&oldval + value);""    while ((readback = atom_cmpxchg((__global uint*)address, oldval, newval)) != oldval) {""        oldval = readback;""        *(float*)&newval = (*(float*)&oldval + value);""    }""    return *(float*)&oldval;""}""""#pragma OPENCL EXTENSION cl_khr_fp64 : enable""#pragma OPENCL EXTENSION cl_khr_int64_base_atomics : enable""double _GfnAtomicAddDouble(__global double* const address, const double value) {""    long oldval, newval, readback;""    *(double*)&oldval = *address;""    *(double*)&newval = (*(double*)&oldval + value);""    while ((readback = atom_cmpxchg((__global long*)address, oldval, newval)) != oldval) {""        oldval = readback;""        *(double*)&newval = (*(double*)&oldval + value);""    }""    return *(double*)&oldval;""}""""__kernel void _kernel_1(int n,double h,__global double * sum,__local double * _cl_local_mem_sum,int _local_i_start,int _local_i_end,int _loop_step) {""int n;""double x;""double h;""double sum = 0;""int _loop_size = (_local_i_end - _local_i_start) / _loop_step;""int i = (get_global_id(0) * _loop_step) + _local_i_start;""if (i <= n) {""{""    x = h * ((double) i - 0.5);""    sum += 4.0 / (1.0 + x * x);""}""}""if (get_global_id(0) < _loop_size) {""_cl_local_mem_sum[get_local_id(0)] = sum;""} else {""_cl_local_mem_sum[get_local_id(0)] = 0;""}""for (int _stride = get_local_size(0)/2; _stride > 0; _stride /= 2) {""barrier(CLK_LOCAL_MEM_FENCE);""if (get_local_id(0) < _stride) {""_cl_local_mem_sum[get_local_id(0)] += _cl_local_mem_sum[get_local_id(0)+_stride];""}""}""if (get_global_id(0) == 0) {""*_local_buffer_sum = 0;""}""barrier(CLK_GLOBAL_MEM_FENCE);""if (get_local_id(0) == 0) {""_GfnAtomicAddDouble(_local_buffer_sum,_cl_local_mem_sum[0]);""}""}";
void _Function_1()
{
    int i;
    int n;
