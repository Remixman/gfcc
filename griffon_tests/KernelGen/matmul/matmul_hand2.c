...
int partial_start(int proc_num, int rank, int part_num, int part_no, int partition_size, int block_size)
{
	return ((rank * proc_num) + part_no) * ceil(partition_size/(float)(proc_num*part_num)) * block_size;
}

int partial_end(int proc_num, int rank, int part_num, int part_no, int partition_size, int block_size)
{
	return ((rank * proc_num) + part_no + 1) * ceil(partition_size/(float)(proc_num*part_num)) * block_size;
	//if (result > partition_size * block_size) result = partition_size * block_size;
	//return result;
}
...
{
  int i, j, k, s;
	int local_start, local_end, par_local_start, par_local_end;
	int A_disp[node_size], par_A_disp[node_size];
	int A_cnts[node_size], par_A_cnts[node_size];
	int C_disp[node_size], par_C_disp[node_size];
	int C_cnts[node_size], par_C_cnts[node_size];
  ...
  size_t work_group_size = 64;
	size_t global_work_size = round_to((local_end - local_start + 1), 
	                                         work_group_size);	
	size_t group_num = global_work_size/work_group_size;
  ...
  //cl_subA = clCreateSubBuffer(cl_A_buf, CL_MEM_READ_ONLY, CL_BUFFER_CREATE_TYPE_REGION, &A_info, &status);
	...
	for (s = 0; s < split_size; s++) {
		...
		par_local_start = partial_start(node_size, rank, split_size, s, nx, nz);
		par_local_end = partial_end(node_size, rank, split_size, s, nx, nz);

		global_work_size = round_to((par_local_end - par_local_start + 1), work_group_size);	
		group_num = global_work_size/work_group_size;

		for (i = 0; i < node_size; ++i) {
			par_A_disp[i] = partial_start(node_size, i, split_size, s, nx, ny);
			par_C_disp[i] = partial_start(node_size, i, split_size, s, nx, nz);
		}
		for (i = 0; i < node_size-1; ++i) {
			par_A_cnts[i] = par_A_disp[i+1] - par_A_disp[i];
			par_C_cnts[i] = par_C_disp[i+1] - par_C_disp[i];
		}
		//par_A_cnts[node_size-1] = (nx * ny) - par_A_disp[node_size-1];
		//par_C_cnts[node_size-1] = (nx * nz) - par_C_disp[node_size-1];

		cl_buffer_region par_A_info;
		par_A_info.origin = (size_t)(par_A_disp[rank] * sizeof(double));
		par_A_info.size = (size_t)(par_A_cnts[rank] * sizeof(double));
		cl_mem cl_par_subA = clCreateSubBuffer(cl_A_buf, CL_MEM_READ_ONLY, 
			CL_BUFFER_CREATE_TYPE_REGION, &par_A_info, &status);

		cl_buffer_region par_C_info;
		par_C_info.origin = (size_t)(par_C_disp[rank] * sizeof(double));
		par_C_info.size = (size_t)(par_C_cnts[rank] * sizeof(double));
		cl_mem cl_par_subC = clCreateSubBuffer(cl_C_buf, CL_MEM_READ_ONLY, 
			CL_BUFFER_CREATE_TYPE_REGION, &par_C_info, &status);

		t0 = get_time();
		MPI_Scatterv((void*)A, par_A_cnts, par_A_disp, MPI_DOUBLE, 
			(void*)(A + par_A_disp[rank]), par_A_cnts[rank], MPI_DOUBLE, 0, MPI_COMM_WORLD);
		t1 = get_time();
		...
		/*if (rank == 0) {
			printf("OFFSET A : %d\n", par_A_disp[rank]);
			printf("UPLOAD A SIZE : %d\n", par_A_cnts[rank]);
		}*/
		status = clEnqueueWriteBuffer(queue, cl_par_subA, CL_FALSE, 0, par_A_cnts[rank] * sizeof(double), 
								  A + par_A_disp[rank], 0, NULL, NULL);

		/*if (rank == 0) {
			printf("par_local_start = %d\n", par_local_start);
			printf("par_local_end = %d\n", par_local_end);
		}*/
		...
		status = clSetKernelArg(kernel, 6, sizeof(int), (void*)&par_local_start);
		_GfnCheckCLStatus(status, "SET KERNEL ARG 6");
		status = clSetKernelArg(kernel, 7, sizeof(int), (void*)&par_local_end);
		...
		/*if (rank == 0) {
			printf("OFFSET C : %d\n", par_C_disp[rank]);
			printf("DOWNLOAD C SIZE : %d\n", par_C_cnts[rank]);
		}*/
		status = clEnqueueReadBuffer(queue, cl_par_subC, CL_FALSE, 0, par_C_cnts[rank] * sizeof(double), 
							     C + par_C_disp[rank], 0, NULL, NULL);

		//clFinish(queue);
		t3 = get_time();
	}

	//printf("RANK[%d] SCATTER TIME : %3.6lf\n", rank, (float)(t1-t0)/1000000);
	//printf("RANK[%d] KERNEL TIME : %3.6lf\n", rank, (float)(t3-t2)/1000000);
