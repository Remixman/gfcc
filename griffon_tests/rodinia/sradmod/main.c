...
int main(int argc, char *argv []){
	...
	inimage = (fp**)malloc(sizeof(fp*) * Nr);
	inimage[0] = (fp*)malloc(sizeof(fp) * Ne);
	for (i = 1; i < Nr; ++i)
		inimage[i] = inimage[i-1] + Nc;
...
#pragma gfn data copyout(image[0:Nr{partition}][0:Nc]) \
	copyin(inimage[0:Nr{partition}][0:Nc]) \
	...
	create(c[0:Nr{partition}][0:Nc])
{
	...
	#pragma gfn parallel_for present(image[0:Nr{partition}][0:Nc]) \
		present(inimage[0:Nr{partition}][0:Nc])
	for (i=0; i<Nr; i++) {
		#pragma gfn loop		
		for (j=0; j<Nc; j++) {
			image[i][j] = exp(inimage[i][j]/255);	// exponentiate input IMAGE and copy to output image
		}
    }
