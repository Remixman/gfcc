...
void _Function_356690()
{
    ...
    _GfnEnqueueScatterND((void *) matrix[0], _cl_mem_matrix, _GFN_TYPE_FLOAT(), _GFN_MEM_READ_WRITE(), 0, (Nsquare) - 1, 1, 0, 0, 1, 1, 2, 0, N, N);
    _GfnFinishDistributeArray();
    /* Compute Workload */
    if (1)
    {
        _kernel = _GfnCreateKernel("_kernel_356690");
