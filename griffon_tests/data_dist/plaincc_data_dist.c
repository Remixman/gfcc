...
/* } */
/*  */
/* __kernel void _kernel_1(__global const int * _local_buffer_B,__global int * _local_buffer_A,__global const int * C,int _local_i_start,int _local_i_end,int _loop_step) { */
/* int j; */
/* int * B = ((void *)_local_buffer_B) - (_local_i_start * sizeof(int)); */
/* int * A = ((void *)_local_buffer_A) - (_local_i_start * sizeof(int)); */
...
/* } */
/* } */
const char *_kernel_1_src = "void _GfnBarrier() {""    barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);""}""""int _GfnAtomicAddInt(__global int* const address, const int value) {""    return atomic_add(address, value);""}""""float _GfnAtomicAddFloat(__global float* const address, const float value) {""    uint oldval, newval, readback;""    *(float*)&oldval = *address;""    *(float*)&newval = (*(float*)&oldval + value);""    while ((readback = atom_cmpxchg((__global uint*)address, oldval, newval)) != oldval) {""        oldval = readback;""        *(float*)&newval = (*(float*)&oldval + value);""    }""    return *(float*)&oldval;""}""""__kernel void _kernel_1(__global const int * _local_buffer_B,__global int * _local_buffer_A,__global const int * C,int _local_i_start,int _local_i_end,int _loop_step) {""int j;""int * B = ((void *)_local_buffer_B) - (_local_i_start * sizeof(int));""int * A = ((void *)_local_buffer_A) - (_local_i_start * sizeof(int));""int _loop_size = (_local_i_end - _local_i_start) / _loop_step;""int i = (get_global_id(0) * _loop_step) + _local_i_start;""if (i <= (1000) - 1) {""{""    A[(i)] = B[(i)];""    for (j = 1;""        j < 4;""        ++j)""    {""        A[(i)] += C[(j)] + C[(j + 3)];""    }""}""}""}";
void _Function_1()
{
    int i;
    int j;
    ...
    if (_gfn_rank == 0)
    {
        _buffer_B = (int *) malloc((sizeof(int) * ((1000))));
        _RecvInputMsg((void *) _buffer_B, (sizeof(int) * ((1000))));
        _buffer_A = (int *) malloc((sizeof(int) * ((1000))));
        _RecvInputMsg((void *) C, (sizeof(int) * ((7))));
    }
    /* Init generated variable */
    _local_i_start = _CalcLocalStartIndex(0, (1000) - 1, _gfn_num_proc, _gfn_rank + 1);
    _local_i_end = _CalcLocalEndIndex(0, (1000) - 1, _gfn_num_proc, _gfn_rank + 1);
    _loop_size = _CalcLoopSize(0, (1000) - 1, 1);
    _loop_step = 1;
    _sub_size_B = _CalcSubSize((1000), _gfn_num_proc, _gfn_rank, ((1) * (1)));
    _local_buffer_B = (int *) malloc(sizeof(int) * _sub_size_B);
    B = ((void *) _local_buffer_B) - (_local_i_start * sizeof(int));
    _CalcCnts((1000), _gfn_num_proc, _B_cnts, ((1) * (1)));
    _CalcDisp((1000), _gfn_num_proc, _B_disp, ((1) * (1)));
    _sub_size_A = _CalcSubSize((1000), _gfn_num_proc, _gfn_rank, ((1) * (1)));
    _local_buffer_A = (int *) malloc(sizeof(int) * _sub_size_A);
    A = ((void *) _local_buffer_A) - (_local_i_start * sizeof(int));
    _CalcCnts((1000), _gfn_num_proc, _A_cnts, ((1) * (1)));
    _CalcDisp((1000), _gfn_num_proc, _A_disp, ((1) * (1)));
    ...
    _GfnCheckCLStatus(_gfn_status, "CREATE BUFFER 0");
    _cl_mem_C = clCreateBuffer(_gfn_context, _get_cl_mem_read_only(), (sizeof(int) * ((7))), 0, &_gfn_status);
    _GfnCheckCLStatus(_gfn_status, "CREATE BUFFER 0");
    /* Send data to all process */
    MPI_Scatterv(_buffer_B, _B_cnts, _B_disp, _get_mpi_int(), _local_buffer_B, _sub_size_B, _get_mpi_int(), 0, _get_mpi_comm_world());
    MPI_Bcast(C, ((7)), _get_mpi_int(), 0, _get_mpi_comm_world());
    /* Compute work-load */
    if (1)
    {
        ...
        _gfn_status = clEnqueueWriteBuffer(_gfn_cmd_queue, _cl_mem_B, _get_cl_true(), 0, sizeof(int) * _sub_size_B, _local_buffer_B, 0, 0, 0);
        _GfnCheckCLStatus(_gfn_status, "WRITE BUFFER _local_buffer_B");
        _gfn_status = clEnqueueWriteBuffer(_gfn_cmd_queue, _cl_mem_C, _get_cl_true(), 0, (sizeof(int) * ((7))), C, 0, 0, 0);
        _GfnCheckCLStatus(_gfn_status, "WRITE BUFFER C");
        _kernel = _CreateKernelFromSource("_kernel_1", _kernel_1_src, _gfn_context, _gfn_device_id);
        _gfn_status = clSetKernelArg(_kernel, 0, sizeof(cl_mem), (void *) &_cl_mem_B);
        _GfnCheckCLStatus(_gfn_status, "SET KERNEL ARG");
        _gfn_status = clSetKernelArg(_kernel, 1, sizeof(cl_mem), (void *) &_cl_mem_A);
        _GfnCheckCLStatus(_gfn_status, "SET KERNEL ARG");
        _gfn_status = clSetKernelArg(_kernel, 2, sizeof(cl_mem), (void *) &_cl_mem_C);
        _GfnCheckCLStatus(_gfn_status, "SET KERNEL ARG");
        ...
        _gfn_status = clEnqueueReadBuffer(_gfn_cmd_queue, _cl_mem_A, _get_cl_true(), 0, sizeof(int) * _sub_size_A, _local_buffer_A, 0, 0, 0);
    ...
    MPI_Gatherv(_local_buffer_A, _sub_size_A, _get_mpi_int(), _buffer_A, _A_cnts, _A_disp, _get_mpi_int(), 0, _get_mpi_comm_world());
    if (_gfn_rank == 0)
    {
        _SendOutputMsg((void *) _buffer_A, (sizeof(int) * ((1000))));
    }
    free(_local_buffer_B);
...
int main()
{
    ...
    _SendCallFuncMsg(1);
    _SendInputMsg((void *) B, (sizeof(int) * ((1000))));
    _SendInputMsg((void *) C, (sizeof(int) * ((7))));
    _RecvOutputMsg((void *) A, (sizeof(int) * ((1000))));
    for (i = 0;
        i < 1000;
        ++i)
    {
        ...
        ++i)
    {
        if (cpuA[i] != A[i])
        {
            ...
            ((0) ? (void) (0) : __assert_fail("0", "data_dist.c", 45, __PRETTY_FUNCTION__));
        }
    }
    printf("TEST PASS!\n");
    return 0;
